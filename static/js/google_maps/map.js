...
Marker.prototype.requestHRAddress = function() {

    var that = this;
    geocoder.geocode( {'location': that.latLng}, function(results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                var res = results[0].formatted_address;
                that.setHRAddress(res);
            } else if (status == google.maps.GeocoderStatus.ZERO_RESULTS) { 
                var res = "Адрес не найден"
                that.setHRAddress(res);
            } else {
...
Marker.prototype.getPolyline = function() {
    
    return this.polyline;
};

Marker.prototype.addPolyline = function(polyline) {

    if ( this.getPolyline().length <= 2 ) {
...
Marker.prototype.delPolyline = function(polyline) {

    var polyArray = this.getPolyline();
    var pos = polyArray.indexOf( polyline );
    polyArray.splice( pos, 1 );
};
...
Marker.prototype.getHRAddress = function() {

    return this.humanReadableAddress;
};

Marker.prototype.setHRAddress = function(address) {

    this.humanReadableAddress = address;
};
...
function deleteMarker(clickedMarkerPosition){
    ...
    */
    ...
    var i, len;
    for (i = 0, len = allPath.length; i < len; i++) {
